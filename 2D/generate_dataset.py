...
# Group naming
# =======================================================
def get_group_name(cfg, seed, ic_val=None, nu_val=None, Re=None,
                   feed_rate=None, kill_rate=None,
                   reactivity=None, critical_wavenumber=None,
                   gamma=None):
    pde = cfg["pde"]
    ...
    if pde == "Burgers":
        ...
        return f"nu{nu_val:.3e}_scale{cfg['vorticity_convection_scale']:.2f}_drag{cfg['drag']:.2f}"
    elif pde == "AllenCahn":
        return f"nu{nu_val:.3e}"
    elif pde == "CahnHilliard":
        return f"nu{nu_val:.3e}_gamma{cfg['gamma']:.1e}"
    else:
...
if __name__ == "__main__":
    ...
    all_trajectories, ic_hashes, trajectory_nus = generate_dataset(
        ...
        vorticity_convection_scale=cfg["vorticity_convection_scale"],
        drag=cfg["drag"],
        gamma=cfg["gamma"],
        c1=cfg["c1"],
        c3=cfg["c3"],
    ...
    with h5py.File(data_path, "w") as h5file:
        idx = 0
        if cfg["pde"] == "Kolmogorov":
            group_name = get_group_name(cfg, seed_list[0], Re=cfg["Re"])
        ...
        elif cfg["pde"] == "FisherKPP":
            for nu_val in cfg["nu"]:
                group_name = get_group_name(cfg, 0, nu_val=nu_val, reactivity=cfg["reactivity"])
                grp = h5file.require_group(group_name)
                for seed in seed_list:
                    u_xt = all_trajectories[idx]
                    ds_name = f"state_seed{seed:03d}"
            ...
            group_name = get_group_name(cfg, 0, reactivity=cfg["reactivity"], critical_wavenumber=cfg["critical_wavenumber"])
            grp = h5file.require_group(group_name)
            for seed in seed_list:
                u_xt = all_trajectories[idx]
                ds_name = f"velocity_seed{seed:03d}"
        ...
        elif cfg["pde"] == "CahnHilliard":
            for nu_val in cfg["nu"]:
                group_name = get_group_name(cfg, 0, nu_val=nu_val, gamma=cfg["gamma"])
                grp = h5file.require_group(group_name)
                for seed in seed_list:
                    u_xt = all_trajectories[idx]
                    ds_name = f"velocity_seed{seed:03d}"
    ...
    def get_sim_metadata(cfg, n_sim, seed_list, ic_hashes=None):
        """Return seed_val, nu_val, ic_val for the given PDE and simulation index."""
        pde = cfg["pde"]

        if pde in ["Burgers", "KuramotoSivashinsky", "FisherKPP",
                "NavierStokesVorticity", "AllenCahn", "CahnHilliard"]:
            nu_val = cfg["nu"][n_sim % len(cfg["nu"])]
        ...
        else:  # Kolmogorov, GrayScott, SwiftHohenberg, etc.
            seed_val = seed_list[n_sim % len(seed_list)]
            nu_val = None
            ic_val = None
    ...
    def create_animation(
        ...
        title = ax.set_title(
            f"{ic} - nu={nu_str} - seed {seed_val:03d} - channel {channel} - t=0"
        )
        ax.set_xlabel("x")
        ax.set_ylabel("y")
    ...
    if cfg["plot_sim"]:
        ...
        selected_simulations = random.sample(
            range(len(all_trajectories)), cfg["plotted_sim"]
        )
        ...
        for n_sim in selected_simulations:
            ...
            for c in range(num_channels):
                ...
                create_animation(
                    ...
                    ic=cfg["ic"],
                    nu_val=nu_val,
                    x_extent=cfg["x_domain_extent"],
                    y_extent=cfg["x_domain_extent"],
                    dt_output=cfg["dt_save"] * cfg["save_freq"],
                    mode="fixed",      # "fixed" or "physical"
